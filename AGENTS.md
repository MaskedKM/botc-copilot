...
### 五大推理公理（Phase 2 推理辅助的规则基础）

> 以下公理基于官方 Wiki（[States](https://wiki.bloodontheclocktower.com/States)、[Rules Explanation](https://wiki.bloodontheclocktower.com/Rules_Explanation)）精确语义。

1. **角色唯一**：同一好人角色至多 1 个。两人声明同一角色 → 至少一假。
2. **座位收缩**：死亡玩家从邻座计算中物理移除，两侧存活者直接并拢。
3. **恶魔 Bluff**：恶魔得知 3 个**不在场**的好人角色。恶魔可自主选择谎报任意角色（不一定是这 3 个），但这 3 个角色的不在场是确定性事实，可用于排除法。
4. **醉毒失能**：醉和毒效果完全相同——玩家**所有能力失效**（不只是一次性技能）。
   - 持续型能力（如 Monk 保护）：醉/毒时失效，恢复后重新生效。
   - 一次性能力（如 Slayer 猎杀）：在醉/毒时尝试使用则**永久消耗**，恢复后也不返还。
   - 对醉/毒玩家使用的能力**正常工作**（如 Empath 读醉汉邻居的阵营是正确的）。
   - 醉/毒**不叠加**，角色变化不影响醉/毒状态（醉/毒是玩家属性）。
   - 毒只在「被投毒的那一夜 + 次日白天」生效，黄昏后解除。
5. **恶魔传承**：恶魔杀死自己 → 一名存活爪牙成为新恶魔（但新恶魔当晚不行动）。注意：传承后邪恶不会立刻战败，新恶魔继续游戏。

### 官方规则要点（开发参考）

> App 须严格遵循以下规则。来源：官方 Wiki + Rulebook。

- **处决 ≠ 死亡**：处决和死亡是独立状态。某些角色能力可处决但不死。
- **死票规则**：死亡玩家**仅有 1 票**，用完不可再用。死亡玩家不能提名，但可以参与讨论和投票。
- **平票规则**：两人得票相同时**无人被处决**。
- **提名限制**：每人每天**只能提名 1 人**，**只能被提名 1 次**。
- **处决阈值**：被提名者需获得 ≥50% 存活玩家赞成票才被处决。
- **7+ 人局**：恶魔和爪牙在首夜互相认识；恶魔额外得知 3 个不在场角色。
- **6 人及以下局**：恶魔不知道爪牙是谁，也不获得 Bluff 信息。
- **Scarlet Woman 触发条件**：恶魔死亡时，SW 存活且 ≥5 名存活玩家 → SW 自动成为新恶魔。
- **首夜特殊性**：第一夜多个角色同时获得信息（与后续夜的行动顺序不同）。Imp 在首夜不杀人。
